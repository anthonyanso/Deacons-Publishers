...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $teachingLevel = mysqli_real_escape_string($connection, $_POST["teachingLevel"]);
    $teachingExperience = mysqli_real_escape_string($connection, $_POST["teachingExperience"]);
    $startDate = mysqli_real_escape_string($connection, $_POST["startDate"]);
    $educationLevel = mysqli_real_escape_string($connection, $_POST["educationLevel"]);
    $joinReason = mysqli_real_escape_string($connection, $_POST["joinReason"]);
    ...
    // Handle CV upload
    $cvPath = handleFileUpload("cv", "cv_", $cvDir);
    if (!$cvPath) {
        echo "Error uploading CV. Please try again.";
        exit;
    }

    // Insert data into the database
    $stmt = $connection->prepare("INSERT INTO teachingApp (fullName, email, phoneNumber, subjects, teachingLevel, teachingExperience, startDate, educationLevel, joinReason, cv) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssss", $fullName, $email, $phoneNumber, $subjects, $teachingLevel, $teachingExperience, $startDate, $educationLevel, $joinReason, $cvPath);

    if ($stmt->execute()) {
        // Email credentials
        $adminEmail = "deaconspublishers@gmail.com";
        $adminPassword = "cjanqzfvgfindzms"; // Use an App Password for security

        // Email to Admin
        $subjectAdmin = "New Teacher Application Submitted";
        $messageAdmin = "
        <!DOCTYPE html>
                        <html>
                        <head>
                        ...
                        <body>
                            ...
                            <!-- User Information -->
                             <ul>
                                ...
                                <li><strong>Teaching Level:</strong> $teachingLevel</li>
                                <li><strong>Teaching Experience:</strong> $teachingExperience years</li>
                                <li><strong>CV:</strong> <a href='http://localhost/deaconspublishers/uploads/$cvPath'>Download CV</a></li>
